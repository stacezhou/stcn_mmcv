...
class STCNModel:
    def __init__(self, para, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        self.low_resolution_mask = para['low_resolution_mask']
    ...
    def do_pass(self,data,it=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.para['amp']):
            out = self.infer_out(data,it=it)
            if self._do_log or self._is_train:
                losses = self.loss_computer.compute({**data, **out}, it)
                if self.low_resolution_mask:
                    out_mask = self.infer_out(lr_data,low_resolution_mask=True,it=it)
                    losses_mask = self.loss_computer.compute({**lr_data, **out_mask}, it)
                    losses_pixel = losses
                    losses = {
                        **losses_pixel,
                        **{f'{k}_mask':v for k,v in losses_mask.items()}
                    }
                    losses['total_loss'] = losses['total_loss'] + losses['total_loss_mask']
    ...
    def infer_out(self, data, it=0, low_resolution_mask=False):
